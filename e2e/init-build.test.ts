...
describe("E2E: Init + Build Happy Path", () => {
  ...
  describe("datasources.append", () => {
    it("appends data from URL using typed client", async () => {
      ...
      // Run build to deploy resources
      const buildResult = await runBuild({ cwd: tempDir });
      expect(buildResult.success).toBe(true);

      // Import the generated client and test append
      const clientPath = path.join(tempDir, "lib/tinybird.ts");
      const clientContent = fs.readFileSync(clientPath, "utf-8");

      // The generated client exports `tinybird` - we need to use it
      // Since we can't directly import the generated TS file, we'll test via the SDK directly
      ...
      // Test append from URL
      const result = await client.datasources.append("page_views", {
        url: "https://example.com/data.csv",
      });

      expect(appendCalled).toBe(true);
      expect(capturedDatasourceName).toBe("page_views");
