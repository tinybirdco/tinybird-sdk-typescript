...
describe('Datasource Generator', () => {
  ...
  describe('Column formatting', () => {
    ...
    it("includes materialized expressions without autogenerated JSON paths", () => {
      const ds = defineDatasource("test_ds", {
        schema: {
          body: t.string(),
          level: t
      ...
      const result = generateDatasource(ds);
      expect(result.content).toContain("body String `json:$.body`");
      expect(result.content).toContain(
      ...
      const ds = defineDatasource("test_ds", {
        schema: {
          ...
          level: t
            .string()
            .materialized("JSONExtractString(body, 'level')")
            .jsonPath("$.ignored_level"),
        },
      });

      const result = generateDatasource(ds);
      expect(result.content).toContain(
        "level String `json:$.ignored_level` MATERIALIZED JSONExtractString(body, 'level')"
      );
    });
