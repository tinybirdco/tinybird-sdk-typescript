...
const EXPECTED_FULL_MIGRATION_OUTPUT = [
  "/**",
  " * Generated by tinybird migrate.",
  " * Review endpoint output schemas and any defaults before production use.",
  " */",
  "",
  'import { createKafkaConnection, defineDatasource, definePipe, defineMaterializedView, defineCopyPipe, node, t, engine } from "@tinybirdco/sdk";',
  "",
  "// Connections",
  "",
  'export const mainKafka = createKafkaConnection("main_kafka", {',
  ...
  "// Pipes",
  "",
  'export const stats = definePipe("stats", {',
  "  nodes: [",
  "    node({",
  '      name: "endpoint",',
  "      sql: `",
...
const EXPECTED_DATASOURCE_ONLY_OUTPUT = [
  "/**",
  " * Generated by tinybird migrate.",
  " * Review endpoint output schemas and any defaults before production use.",
  " */",
...
`
    ...
    expect(output).toBe(EXPECTED_FULL_MIGRATION_OUTPUT);
  });
...
`
    ...
    const output = fs.readFileSync(result.outputPath, "utf-8");
    expect(output).toBe(EXPECTED_DATASOURCE_ONLY_OUTPUT);
  });
...
`
    ...
    expect(result.success).toBe(true);
    expect(result.errors).toHaveLength(0);
    expect(result.outputContent).toBe(EXPECTED_DATASOURCE_ONLY_OUTPUT);
